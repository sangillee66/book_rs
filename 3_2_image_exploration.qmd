...
#| fig-dpi: 600
library(ggcorrplot)
ggcorrplot(cor_matrix_1, hc.order = TRUE, tl.cex = 18, lab = TRUE, lab_size = 6, outline.color = "gray80", colors = c("#6D9EC1", "white", "#E46726"), digits = 3) + 
...
#| fig-dpi: 600
library(ggcorrplot)
ggcorrplot(cor_matrix_2, hc.order = TRUE, tl.cex = 18, lab = TRUE, lab_size = 6, outline.color = "gray80", colors = c("#6D9EC1", "white", "#E46726"), digits = 3) + 
  labs(caption = "SANG-IL LEE, Geography Education at SNU, 2025")
```
...
#| fig-height: 12
#| fig-dpi: 600
my_map <- tm_shape(level2_landsat_seoul_st_mask$B5, raster.downsample = FALSE) + 
  tm_raster(style = "cont", palette = "-Greys", title = "DN") +
...
profile <- level2_landsat_seoul_st_mask$B5 |> 
  extract(points_sf) |> 
  mutate(
...
# 시각화
ggplot(profile, aes(x = distance, y = B5)) +
  geom_line() +
  # theme_minimal() +
  theme(
    plot.title = element_text(size = 30), 
...
서울시 4곳의 분광 프로파일을 추출하고자 한다. 우선 위치를 결정하고 지도에 나타낸다.
...
df_spec <- spec_profile |> 
  ...
  mutate(
    band = fct(band, levels = str_c("B", 1:7)),
    ID = as_factor(ID)
  )
